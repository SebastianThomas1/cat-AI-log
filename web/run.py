...
expansions_df = pd.read_csv('data/abbreviations.csv', sep=';', index_col='abbreviation')
expansions_df.set_index(expansions_df.index.str.replace(r'\.$', '').str.lower(), inplace=True)
expansions = dict(expansions_df['expansion'])

replacements_dm_ifa_df = pd.read_csv('data/replacements_dm_ifa.csv', sep=';', index_col='abbreviation_dm')
replacements_dm_ifa_df.set_index(replacements_dm_ifa_df.index.str.replace(r'\.$', '').str.lower(), inplace=True)
replacements_dm_ifa = dict(replacements_dm_ifa_df['abbreviation_ifa'])

replacements_dosage_form_abbreviation_df = pd.read_csv('data/dosage_forms_ifa.csv', sep=';',
                                                       index_col='dosage form')
replacements_dosage_form_abbreviation_df.set_index(replacements_dosage_form_abbreviation_df.index.str.replace(r'\.$', '').str.lower(),
                                                   inplace=True)
replacements_dosage_form_abbreviation = dict(replacements_dosage_form_abbreviation_df['abbreviation'])
replacement_dicts=[expansions, replacements_dm_ifa, replacements_dosage_form_abbreviation]

dosage_forms_ifa = pd.read_csv('data/dosage_forms_ifa.csv', sep=';', index_col='abbreviation')
...
def run():
    ...
    if query is not None:
        ...
        corrected_query = spelling_corrector._transform(query) if correct_spelling else query
        preprocessed_query = preprocessor.transform([corrected_query])[0]

        prediction = label_encoder.inverse_transform(classifier.predict([preprocessed_query]))[0]
        ...
        extracted_features = dict(extract_features_mira(clean_mira([query], replacement_dicts=replacement_dicts),
                                                        token_lists, token_feature_names).iloc[0])
        extracted_features = [(translation.get(item[0], 'andere'), item[1]) for item in extracted_features.items()
                              if not(type(item[1]) == np.float64 and np.isnan(item[1]) or item[1] is pd.NA
                                     or item[0] in ['manufacturer article number', 'additional fee', 'ambulant',
                                                    'stationary'])]

        indices = search_engine.recommend(corrected_query, max_count=None, output='indices',
                                          correct_spelling=correct_spelling, include_all=include_all)
        recommendations = mira.iloc[indices][['article', 'prediction print', 'certainty print']].values
    else:
        prediction = None
        extracted_features = []
        recommendations = []
